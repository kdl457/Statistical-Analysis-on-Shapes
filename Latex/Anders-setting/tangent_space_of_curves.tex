...
\end{align*}
$\I$ is the space of \textit{immersions} of the unit circle into $\R^2$, and $\mathcal{I}$ is then this space modulo reparametrization, i.e., we identify two objects $a,b \in \I$ in $\mathcal{I}$ if $a=b \circ \phi$, where $\phi \in \mathrm{Diff}(\S^1)$ is a diffeomorphism on the unit circle.
...
In the following we shall often consider a path of curves which we denote by $q(t,\theta)$; thus, for every fixed $t$ we have a parametrized shape $q(t, \blank)$. We denote the time and the parameter derivates as $q_t$ and $q_{\theta}$ respectively.
